...
INCLUDE_LINE_PATT = re.compile(r'#include (["<][\w/]+.?\w+.?\w+[">])(.*)')
NOLINT_TEXT = " // NOLINT"
...
def normalize_include_line(include, comment: str):
    # quote = include[0] == '"'
    header = include[1:-1]
    if comment.strip().startswith("// for "):
        comment = ""
    assert QT_HEADER_MAPPINGS is not None
...
def normalize_header_includes(fentry: Path, dry=False, strip=False):
    lines = []
    ...
    if not dry and lines:
        fentry_new = Path(f"{fentry}.new")
        with open(fentry_new, "w", encoding="utf-8") as fout:
    ...
    for ent in args.files_or_dirs:
        ...
        if ent.is_file():
            if ent.suffix in C_OR_CPP_EXTENSIONS:
                ...
                logging.warning(f"Non-C-family file: {ent}")
        elif ent.is_dir():
