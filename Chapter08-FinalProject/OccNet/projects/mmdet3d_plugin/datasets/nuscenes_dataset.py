...
class CustomNuScenesDataset(NuScenesDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.train_with_partial_data:
            self.valid_scenes = set(
                np.load(
                    os.path.join(
                        "data/nuscenes_train_splits",
                        f"split_{split_divisor}.npy",  # noqa E501
                    )
                )
            )
    ...
    def load_annotations(self, ann_file):
        ...
        if self.train_with_partial_data:
            data_infos = [
                ...
                if info["scene_name"] in self.valid_scenes  # noqa E501
            ]
    ...
    def prepare_train_data(self, index):
        ...
        for i in prev_indexs_list:
            ...
            if (
                ...
                if self.filter_empty_gt and (
                    example is None
                    or ~(example["gt_labels_3d"]._data != -1).any()  # noqa E501
                ):
                    return None
                frame_idx = input_dict["frame_idx"]
    ...
    def union2one(self, queue):
        ...
        for i, each in enumerate(queue):
            ...
            if i == 0:
                ...
                metas_map[i]["can_bus"][
                    :3
                ] -= prev_pos  # the delta position of adjacent timestamps
                metas_map[i]["can_bus"][
                    -1
                ] -= prev_angle  # the delta orientation of adjacent timestamps
        ...
        if self.use_occ_gts:
            queue[-1]["gt_bboxes_3d"] = DC(
                [each["gt_bboxes_3d"].data for each in queue], cpu_only=True
            )
            queue[-1]["gt_labels_3d"] = DC(
    ...
    def evaluate_occ_iou(
        ...
        # set the metrics
        self.eval_metrics = SSCMetrics(
            self.occupancy_classes + 1, occ_type=self.occ_type
        )
        # loop the dataset
        for index in tqdm(range(len(occupancy_results))):
            ...
            gt_occupancy = (
                np.ones(self.voxel_num, dtype=np.uint8)
                * self.occupancy_classes  # noqa E501
            )
            ...
            if flow_results is not None:
                ...
                flow_true, flow_pred = self.parse_flow_info(
                    occ_gt_sparse,
                    occ_pred_sparse,
                    flow_gt_sparse,
                    flow_pred_sparse,  # noqa E501
                )
            ...
            if show_dir and index % save_interval == 0:
                ...
                if scene_name in save_scene_names:
                    save_result = True
                if save_result:
                    occ_gt_save_dir = os.path.join(
                        show_dir, thre_str, scene_name, "occ_gts"
                    )
                    occ_pred_save_dir = os.path.join(
                        show_dir, thre_str, scene_name, "occ_preds"
                    )
                    image_save_dir = os.path.join(
                        show_dir, thre_str, scene_name, "images"
                    )
                    ...
                    np.save(
                        osp.join(
                            occ_gt_save_dir,
                            "{:03d}_occ.npy".format(frame_idx),  # noqa E501
                        ),  # noqa E501
                        occ_gt_sparse,
                    )
                    np.save(
                    ...
                    if flow_results is not None:
                        np.save(
                            osp.join(
                                occ_gt_save_dir,
                                "{:03d}_flow.npy".format(frame_idx),  # noqa E501
                            ),
                            flow_gt_sparse,
                        )
                        np.save(
                    ...
                    image_save_path = osp.join(
                        image_save_dir, "{:03d}.png".format(frame_idx)
                    )
                    if "surround_image_path" in info:
                        shutil.copyfile(
                            info["surround_image_path"], image_save_path
                        )  # noqa E501
        ...
        for i, class_name in enumerate(self.occupancy_class_names):
            print(
                "miou/{}: {:.2f}".format(
                    class_name, eval_resuslt["iou_ssc"][i]
                )  # noqa E501
            )  # noqa E501
        ...
        # flow_states=['flow_distance_sta', 'flow_distance_mov', 'flow_distance_all'] # noqa E501
        ...
        if runner is not None:
            for i, class_name in enumerate(self.occupancy_class_names):
                runner.log_buffer.output[class_name] = eval_resuslt["iou_ssc"][
                    i
                ]  # noqa E501
    ...
    def obtain_occ_pred_valid(self, occ_pred_sparse, invalid_occupancy):
        ...
        occ_pred_full = (
            np.ones(self.voxel_num, dtype=np.uint8) * self.occupancy_classes
        )  # noqa E501
        occ_pred_full[occ_index] = occ_class
        valid_mask = (occ_pred_full != self.occupancy_classes) & (
            invalid_occupancy != 255
        )
        occ_pred_valid_index = np.where(valid_mask)[0]
        occ_pred_valid_label = occ_pred_full[valid_mask]
        occ_pred_valid = np.stack(
            [occ_pred_valid_index, occ_pred_valid_label], axis=-1
        )  # noqa E501
        return occ_pred_valid

    def obtain_flow_pred_valid(
        self, occ_pred_sparse, flow_pred_sparse, invalid_occupancy
    ):
        occ_index, occ_class = occ_pred_sparse[:, 0], occ_pred_sparse[:, 1]
        occ_pred_full = (
            np.ones(self.voxel_num, dtype=np.uint8) * self.occupancy_classes
    ...
    def parse_flow_info(self, occ_gt, occ_pred, flow_gt, flow_pred):
        ...
        flow_pred_full = np.expand_dims(
            flow_pred_full, axis=0
        )  # (1, 640000, 2) # noqa E501
