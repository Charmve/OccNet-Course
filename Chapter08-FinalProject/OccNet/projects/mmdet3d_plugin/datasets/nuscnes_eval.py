...
from nuscenes.eval.detection.render import class_pr_curve  # noqa E501
from nuscenes.eval.detection.render import dist_pr_curve, summary_plot
...
def class_tp_curve(
    ...
    :param savepath: If given, saves the the rendering here instead of displaying. # noqa E501
    ...
    if min_recall_ind <= md.max_recall_ind:
        # For traffic_cone and barrier only a subset of the metrics are plotted. # noqa E501
        rel_metrics = [
            m
            for m in TP_METRICS
        ...
        ylimit = (
            max(
                [
                    max(
                        getattr(md, metric)[
                            min_recall_ind : md.max_recall_ind + 1  # noqa E203
                        ]  # noqa E501
                    )  # noqa E501
...
class DetectionBox_modified(DetectionBox):
    def __init__(
        self, *args, token=None, visibility=None, index=None, **kwargs
    ):  # noqa E501
        """
    ...
    def deserialize(cls, content: dict):
        ...
        return cls(
            ...
            num_pts=-1
            if "num_pts" not in content
            else int(content["num_pts"]),  # noqa E501
...
def center_in_image(
    ...
    """
    Check if a box is visible inside an image without accounting for occlusions. # noqa E501
...
def load_gt_2(
    ...
    if verbose:
        print(
            "Loading annotations for {} split from nuScenes version: {}".format(  # noqa E501
    ...
    if eval_split in {"train", "val", "train_detect", "train_track"}:
        assert version.endswith(
            "trainval"
        ), "Error: Requested split {} which is not compatible with NuScenes version {}".format(  # noqa E501
            eval_split, version
        )
        ...
        raise ValueError(
            "Error: Requested split {} which this function cannot map to the correct NuScenes version.".format(  # noqa E501
                eval_split
            )
        )
    ...
    if eval_split == "test":
        ...
        ), "Error: You are trying to evaluate on the test set but you do not have the annotations!"  # noqa E501
    index_map = {}
        ...
        for sample_annotation_token in sample_annotation_tokens:
            ...
            if box_cls == DetectionBox_modified:
                ...
                if attr_count == 0:
                    ...
                    raise Exception(
                        "Error: GT annotations must not have more than one attribute!"  # noqa E501
                    )
                ...
                sample_boxes.append(
                    box_cls(
                        ...
                        detection_score=-1.0,  # GT samples do not have a score. # noqa E501
        ...
        for box in eval_boxes[sample_token]:
            ...
            for cam in cams:
                ...
                if center_in_image(
                    ...
                    count += 1
                # if exist_corners_in_image_but_not_all(new_box, cam_intrinsic, imsize, vis_level=BoxVisibility.ANY):  # noqa E501
                #    count += 1
            ...
            if count > 1:
                with open("center_overlap.txt", "a") as f:
                    try:
                        ...
                        logging.error(
                            f"write {box.token} into center_overlap.txt failed."  # noqa E501
                        )
                        pass
                filtered_boxes.append(box)
        anns_filter += len(filtered_boxes)
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(
        ...
        :param eval_set: The dataset split to evaluate on, e.g. train, val or test. # noqa E501
        ...
        assert os.path.exists(
            result_path
        ), "Error: The result file does not exist!"  # noqa E501
    ...
    def update_gt(self, type_="vis", visibility="1", index=1):
        if type_ == "vis":
            ...
            if self.visibility_test:
                """
                [{
                    'description': 'visibility of whole object is between 0 and 40%', # noqa E501
                    'token': '1',
                    'level': 'v0-40'
                 },
                 {
                    'description': 'visibility of whole object is between 40 and 60%', # noqa E501
                    'token': '2',
                    'level': 'v40-60'
                },
                {'description': 'visibility of whole object is between 60 and 80%', # noqa E501
                'token': '3',
                'level': 'v60-80'},
                {'description': 'visibility of whole object is between 80 and 100%', # noqa E501
                'token': '4',
                'level': 'v80-100'}]
                """
    ...
    def evaluate(self) -> Tuple[DetectionMetrics, DetectionMetricDataList]:
        ...
        # -----------------------------------
        # Step 1: Accumulate metric data for all classes and distance thresholds. # noqa E501
        # -----------------------------------
        if self.verbose:
        ...
        for class_name in self.cfg.class_names:
            ...
            for dist_th in self.cfg.dist_ths:
                ...
                ap = calc_ap(
                    metric_data, self.cfg.min_recall, self.cfg.min_precision
                )  # noqa E501
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Folder to store result metrics, graphs and example visualizations.",  # noqa E501
    )
    parser.add_argument(
        "--eval_set",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Which version of the nuScenes dataset to evaluate on, e.g. v1.0-trainval.",  # noqa E501
    )
    parser.add_argument(
        "--config_path",
        type=str,
