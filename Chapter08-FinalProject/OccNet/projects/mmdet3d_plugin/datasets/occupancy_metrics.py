...
class SSCMetrics:
    ...
    def add_batch(
        self, y_pred, y_true, flow_pred=None, flow_true=None, visible_mask=None
    ):
    ...
    def get_stats(self):
        miou = (
            np.diag(self.hist)
            / (self.hist.sum(1) + self.hist.sum(0) - np.diag(self.hist) + 1e-6)
            * 100.0
        )
    ...
    def obtain_masked_distanced_voxel(self):
        ...
        points_x = (index_xyz[:, 0] + 0.5) / self.occ_xdim * (
            self.point_cloud_range[3] - self.point_cloud_range[0]
        ) + self.point_cloud_range[0]
        points_y = (index_xyz[:, 1] + 0.5) / self.occ_ydim * (
            self.point_cloud_range[4] - self.point_cloud_range[1]
        ) + self.point_cloud_range[1]
        points_z = (index_xyz[:, 2] + 0.5) / self.occ_zdim * (
            self.point_cloud_range[5] - self.point_cloud_range[2]
        ) + self.point_cloud_range[2]
        points = np.concatenate(
            [
                points_x.reshape(-1, 1),
                points_y.reshape(-1, 1),
                points_z.reshape(-1, 1),
            ],  # noqa E501
            axis=-1,
        )
