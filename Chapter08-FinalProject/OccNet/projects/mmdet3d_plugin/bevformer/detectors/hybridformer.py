...
class HybridFormer(MVXTwoStageDetector):
    """HybridFormer
    Args:
        video_test_mode (bool): Decide whether to use temporal
    ...
    def __init__(
        ...
        self.only_occ = (
            only_occ  # only output occupancy, do not consider the detection result
        )
    ...
    def extract_img_feat(self, img, img_metas, len_queue=None):
        ...
        if self.with_img_neck:
            img_feats = self.img_neck(
                img_feats
            )  # for num_outs=4, [(bs, _dim_, hi, wi)]
    ...
    def extract_pts_feat(self, pts, img_feats, img_metas):
        ...
        for pts_sample in pts:
            ...
            for p in pts_sample:
                coors_sample.append(
                    self.voxelize([p], pc_range, voxel_size, (x_dim, y_dim, z_dim))
                )
    ...
    def forward_pts_train(
        ...
        if self.dataset_type == "semantic_kitti":
            ...
            losses = self.pts_bbox_head.loss_semantic_kitti(
                *loss_inputs, img_metas=img_metas
            )
        else:
            if self.only_det:
                loss_inputs = [gt_bboxes_3d, gt_labels_3d, outs]
                losses = self.pts_bbox_head.loss_only_detection(
                    *loss_inputs, img_metas=img_metas
                )
            elif self.only_occ:
                loss_inputs = [
                ...
                losses = self.pts_bbox_head.loss_only_occupancy(
                    *loss_inputs, img_metas=img_metas
                )
    ...
    def forward_train(
        ...
        **kwargs
        ...
        if self.keep_bev_history:
            prev_bev = self.obtain_all_history_bev(
                prev_img, prev_img_metas
            )  # list(Tensor), Tensor: (bs, num_query, embed_dims)
        else:
    ...
    def simple_test(
        ...
        if occ_results["occupancy_preds"] is not None:
            occ_results = self.pts_bbox_head.get_occupancy_prediction(
                occ_results, occ_threshold
            )
