...
def custom_train_detector(
    ...
    data_loaders = [
        build_dataloader(
            ...
            dist=distributed,
            seed=cfg.seed,
            shuffler_sampler=cfg.data.shuffler_sampler,  # dict(type='DistributedGroupSampler'), # noqa:E501
            nonshuffler_sampler=cfg.data.nonshuffler_sampler,  # dict(type='DistributedSampler'), # noqa:E501
        )
        for ds in dataset
    ]
    ...
    if validate:
        ...
        eval_cfg["jsonfile_prefix"] = osp.join(
            "val",
            cfg.work_dir,
            time.ctime().replace(" ", "_").replace(":", "_"),  # noqa:E501
        )
