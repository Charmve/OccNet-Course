...
def custom_multi_gpu_test(
    model, data_loader, tmpdir=None, gpu_collect=False, occ_threshold=0.25
):
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            result, occ_results = model(
                return_loss=False, rescale=True, occ_threshold=occ_threshold, **data
            )
            ...
            #    assert False, 'this code is for instance segmentation, which our code will not utilize.' # noqa:E501
    ...
    if gpu_collect:
        ...
        if len(occupancy_results):
            ...
            occupancy_results = collect_results_cpu(
                occupancy_results, len(dataset), tmpdir
            )
        else:
