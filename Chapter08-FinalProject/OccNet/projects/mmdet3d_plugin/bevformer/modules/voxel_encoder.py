...
class VoxelFormerEncoder(TransformerLayerSequence):
    ...
    def get_reference_points(
        ...
        if dim == "3d":
            ...
            zs = (
                torch.linspace(0.5, bev_z - 0.5, bev_z, dtype=dtype, device=device)
                .view(1, bev_z, 1, 1)
                .expand(1, bev_z, bev_h, bev_w)
                / bev_z
            )
            ys = (
            ...
            ref_3d = (
                ref_3d.permute(0, 4, 1, 2, 3).flatten(2).permute(0, 2, 1)
            )  # (D, num_query, 3)
            ...
            if num_points_in_voxel > 1:
                ...
                # the offset of sampling point from the voxel center
                # is in the range [-delta, delta]
                zs_offset = torch.linspace(
                    -delta_z, delta_z, num + 2, dtype=dtype, device=device
                )[1:-1]
                zs_offset = zs_offset.view(num, 1, 1, 1).expand(
                    num, bev_z, bev_h, bev_w
                )

                ys_offset = torch.linspace(
                    -delta_y, delta_y, num + 2, dtype=dtype, device=device
                )[1:-1]
                ys_offset = ys_offset.view(num, 1, 1, 1).expand(
                    num, bev_z, bev_h, bev_w
                )

                xs_offset = torch.linspace(
                    -delta_x, delta_x, num + 2, dtype=dtype, device=device
                )[1:-1]
                xs_offset = xs_offset.view(num, 1, 1, 1).expand(
                    num, bev_z, bev_h, bev_w
                )

                offset_3d = torch.stack(
                    (xs_offset, ys_offset, zs_offset), -1
                )  # (num, bev_z, bev_h, bev_w, 3)
                offset_3d = (
                    offset_3d.permute(0, 4, 1, 2, 3).flatten(2).permute(0, 2, 1)
                )  # (num, num_query, 3)
                offset_3d = offset_3d[None].repeat(
                    bs, 1, 1, 1
                )  # (bs, num, num_query, 3)
                ...
                # zs_offset = torch.cat([zs_offset_m, zs_offset_p], axis=0).view(num, 1, 1, 1) \
                #    .expand(num, bev_z, bev_h, bev_w)
                ...
                # ys_offset = torch.cat([ys_offset_m, ys_offset_p], axis=0).view(num, 1, 1, 1) \
                #    .expand(num, bev_z, bev_h, bev_w)
                ...
                # xs_offset = torch.cat([xs_offset_m, xs_offset_p], axis=0).view(num, 1, 1, 1) \
                #    .expand(num, bev_z, bev_h, bev_w)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # bug: this code should be 'shift_ref_2d = ref_2d.clone()',
        # we keep this bug for reproducing our results in paper.
