...
class HybridPerceptionTransformer(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.voxel_encoder_num = (
            self.encoder_block_num - 1
        )  # the number of voxel encoders
    ...
    def define_voxel_encoder(self, model_config, pos_config):
        for i in range(1, self.encoder_block_num):
            # set model config
            model_config.transformerlayers.attn_cfgs[
                0
            ].embed_dims = self.encoder_embed_dims[i]
            model_config.transformerlayers.attn_cfgs[
                1
            ].deformable_attention.embed_dims = self.encoder_embed_dims[i]
            model_config.transformerlayers.attn_cfgs[
                1
            ].embed_dims = self.encoder_embed_dims[i]
            model_config.transformerlayers.ffn_cfgs.embed_dims = self.encoder_embed_dims[
                i
            ]
            model_config.transformerlayers.ffn_cfgs.feedforward_channels = (
                self.encoder_embed_dims[i] * 2
            )
            model_config.transformerlayers.feedforward_channels = (
                self.encoder_embed_dims[i] * 2
            )
            self.encoders.append(build_transformer_layer_sequence(model_config))
    ...
    def init_layers(self):
        ...
        for i in range(self.encoder_block_num - 1):
            ...
            block = nn.Sequential(
                nn.Linear(fc1, fc2), nn.ReLU(inplace=True), nn.LayerNorm(fc2),
            )
    ...
    def init_weights(self):
        ...
        for block in self.bev_voxel_transfers:
            xavier_init(block, distribution="uniform", bias=0.0)
        ...
        for block_index in range(self.encoder_block_num):
            ...
            if block_bev_z == 1:
                bev_mask = torch.zeros(
                    (bs, bev_h, bev_w), device=bev_queries.device
                ).to(bev_queries.dtype)
            else:
                bev_mask = torch.zeros(
                    (bs, block_bev_z, bev_h, bev_w), device=bev_queries.device
                ).to(bev_queries.dtype)
            pos = self.positional_encodings[block_index](bev_mask).to(
                bev_queries.dtype
            )  # (bs, embed_dims, h, w)
            ...
            if prev_bev is not None:  # (bs, num_query, embed_dims)
                ...
                if block_bev_z == 1:  # 2D BEV
                    if stage_prev_bev.shape[1] == bev_h * bev_w:
                        stage_prev_bev = stage_prev_bev.permute(
                            1, 0, 2
                        )  # (num_query, bs, embed_dims)
                    ...
                    if self.rotate_prev_bev:  # revise for 3D feature map
                        for i in range(bs):
                            ...
                            tmp_prev_bev = (
                                stage_prev_bev[:, i]
                                .reshape(block_bev_z, bev_h, bev_w, -1)
                                .permute(3, 0, 1, 2)
            ...
            if (
                block_index < self.encoder_block_num - 1
            ):  # bev-> voxel or voxel_i -> voxel_j
                bev_queries = output.view(
                    block_bev_z, bev_h, bev_w, bs, self.encoder_embed_dims[block_index]
                )
                bev_queries = bev_queries.permute(1, 2, 3, 0, 4)
                bev_queries = bev_queries.flatten(
                    3
                )  # (bev_h, bev_w, bs, embed_dims1*z1)
                bev_queries = self.bev_voxel_transfers[block_index](
                    bev_queries
                )  # (bev_h, bev_w, bs, embed_dims2*z2)
                bev_queries = bev_queries.view(
                    bev_h,
                    bev_w,
                    bs,
                    self.feature_map_z[block_index + 1],
                    self.encoder_embed_dims[block_index + 1],
                )
                bev_queries = bev_queries.permute(3, 0, 1, 2, 4)
                bev_queries = bev_queries.reshape(
                    -1, bs, self.encoder_embed_dims[block_index + 1]
                )  # (num_query, bs, embed_dims)
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.decoder_on_bev:
            bev_embed = voxel_embed.view(
                self.feature_map_z[-1], bev_h, bev_w, bs, self.encoder_embed_dims[-1]
            )
