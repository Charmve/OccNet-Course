    ...
    def forward(
        ...
        **kwargs,
        ...
        for i, mask_per_img in enumerate(
            bev_mask
        ):  # bev_mask.shape  (num_cam, bs, num_query, D)
            index_query_per_img = (
    ...
    def forward(
        ...
        **kwargs,
        ...
        if reference_points.shape[-1] == 2:
            """
            For each BEV query, it owns `num_Z_anchors` in 3D space that having
            different heights. After proejcting, each BEV query has `num_Z_anchors`
            reference points in each 2D image. For each referent point, we sample
            `num_points` sampling points. For `num_Z_anchors` reference points,
