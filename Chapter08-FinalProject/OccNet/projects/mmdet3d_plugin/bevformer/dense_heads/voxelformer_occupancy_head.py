...
class VoxelFormerOccupancyHead(DETRHead):
    ...
    def _init_layers(self):
        ...
        if self.bev_z == self.occ_zdim:
            self.occ_proj = Linear(
                self.embed_dims, self.occ_dims
            )  # the voxel size is consistent with the occupancy size
        else:
            self.occ_proj = Linear(
                self.bev_z * self.embed_dims, self.occ_dims * self.occ_zdim
            )
        # occupancy branch
        if self.loss_occupancy is not None:
            occ_branch = []
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None, only_bev=False):
        ...
        voxel_mask = torch.zeros(
            (bs, self.bev_z, self.bev_h, self.bev_w), device=voxel_queries.device
        ).to(dtype)
        voxel_pos = self.positional_encoding(voxel_mask).to(
            dtype
        )  # (bs, embed_dims, d, h, w)
        ...
        if (
            ...
            bev_for_occ = bev_for_occ.view(
                bs * seq_len, self.bev_z, self.bev_h, self.bev_w, self.embed_dims
            )
            if self.bev_z == self.occ_zdim:
