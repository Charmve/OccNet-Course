...
class HybridFormerOccupancyHead(DETRHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.positional_encoding = (
            None  # we define them in the transformer encoder # noqa:E501
        )
    ...
    def _init_layers(self):
        ...
        if self.transformer.decoder is not None:  # 3D detection setting
            cls_branch = []
            ...
            if not self.as_two_stage:
                ...
                self.query_embedding = nn.Embedding(
                    self.num_query, self.box_query_dims * 2
                )
        else:
            self.bev_embedding = nn.Embedding(self.bev_num, self.embed_dims)

        # occupancy branch
        if self.bev_z == self.occ_zdim or self.use_fine_occ:
            self.occ_proj = Linear(
                self.last_voxel_dims, self.occ_dims
            )  # the voxel size is consistent with the occupancy size
        else:
            self.occ_proj = Linear(
                self.bev_z * self.last_voxel_dims,
                self.occ_dims * self.occ_zdim,  # noqa:E501
            )
        if self.loss_occupancy is not None:
            occ_branch = []
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None, only_bev=False):
        ...
        bev_queries = self.bev_embedding.weight.to(
            dtype
        )  # start from the bev embeddings, end with the voxel embeddings
        ...
        if (
            ...
            if self.use_fine_occ:
                bev_for_occ = bev_for_occ.view(
                    bs * seq_len, self.bev_z, self.bev_h, self.bev_w, -1
                )
                bev_for_occ = bev_for_occ.permute(0, 4, 1, 2, 3)
                bev_for_occ = self.up_sample(
                    bev_for_occ
                )  # (bs*seq_len, C, occ_z, occ_y, occ_x)  3D upsampling
                ...
                if self.bev_z == self.occ_zdim:
                    occ_pred = self.occ_proj(
                        bev_for_occ
                    )  # (bs*seq_len, bev_z, bev_h, bev_w, self.occ_dims)
                else:
                    bev_for_occ = bev_for_occ.permute(0, 2, 3, 1, 4).flatten(
                        3
                    )  # (bs*seq_len, bev_h, bev_w, -1)
                    occ_pred = self.occ_proj(bev_for_occ)
            ...
            occ_pred = occ_pred.reshape(
                bs * seq_len, self.occ_zdim, -1, self.occ_dims
            )  # bs*seq_len, z, y*x, dim
            ...
            bev_embed = bev_embed.permute(
                1, 0, 2
            )  # [bs, bev_z*bev_h*bev_w, embed_dims]
            ...
            if self.bev_z == self.occ_zdim:
                ...
                occ_pred = self.occ_proj(bev_for_occ)
                occ_pred = occ_pred.view(
                    bs * seq_len, self.bev_h, self.bev_w, self.occ_zdim, self.occ_dims
                )
                occ_pred = occ_pred.permute(0, 3, 1, 2, 4)
            occ_pred = occ_pred.reshape(
                bs * seq_len, self.occ_zdim, -1, self.occ_dims
    ...
    def loss_single(
        ...
        # noqa:E501
        # transfer gt anno(cx, cy, cz, w, l, h, rot, vx, vy) to
    ...
    def loss(
        ...
        gt_occupancy = (
            torch.ones((bs * seq_len, self.occ_num), dtype=torch.long)
        ...
        for sample_id in range(len(temporal_gt_bboxes_list)):
            for frame_id in range(seq_len):
                ...
                gt_occupancy[sample_id * seq_len + frame_id][[occ_gt[:, 0]]] = occ_gt[
                    :, 1
                ]
        ...
        ]  # list(tensor), tensor.shape: (num_box, 9)
        ...
        )  # (bs*seq_len*occ_zdim*occ_ydim*occ_xdim, 11)
        ...
        if enc_cls_scores is not None:
            ...
            enc_loss_cls, enc_losses_bbox, enc_losses_occupancy = self.loss_single(
                enc_cls_scores,
                enc_bbox_preds,
                enc_occupancy_preds,
                gt_bboxes_list,
    ...
    def loss_only_detection(
        ...
        losses_cls, losses_bbox = multi_apply(
            self.loss_single_for_detection,
            all_cls_scores,
            all_bbox_preds,
            all_gt_bboxes_list,
            all_gt_labels_list,
        ...
        if enc_cls_scores is not None:
            ...
            enc_loss_cls, enc_losses_bbox = self.loss_single_for_detection(
                enc_cls_scores,
                enc_bbox_preds,
                gt_bboxes_list,
                binary_labels_list,
    ...
    def get_occupancy_prediction(self, occ_results, occ_threshold=0.25):
        ...
        if self.occ_loss_type == "focal_loss":
            ...
            (occ_index,) = torch.where(
                occ_class < self.occupancy_classes
            )  # torch.int64
