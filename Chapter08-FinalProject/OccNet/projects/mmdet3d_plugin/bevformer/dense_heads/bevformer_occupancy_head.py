...
class BEVFormerOccupancyHead(DETRHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.occ_xdim = int(
            (point_cloud_range[3] - point_cloud_range[0]) / occupancy_size[0]
        )
        self.occ_ydim = int(
            (point_cloud_range[4] - point_cloud_range[1]) / occupancy_size[1]
        )
        self.occ_zdim = int(
            (point_cloud_range[5] - point_cloud_range[2]) / occupancy_size[2]
        )
    ...
    def _init_layers(self):
        ...
        if self.use_fine_occ:
            self.occ_proj = Linear(
                self.embed_dims, self.occ_dims * self.occ_zdim // 2
            )  # noqa:E501
        else:
            self.occ_proj = Linear(
                self.embed_dims, self.occ_dims * self.occ_zdim
            )  # noqa:E501
        ...
        if self.use_fine_occ:
            self.up_sample = nn.Upsample(
                size=(self.occ_zdim, self.occ_ydim, self.occ_xdim),
                mode="trilinear",
                align_corners=True,
            )
    ...
    def occupancy_aggregation(self, occ_pred):
        ...
        Args:
            # noqa: E501
            occ_pred:
                occupancy prediction with shape (bs, seq_len, z, x, y, dim)
        """
        ...
        ref_3d = self.transformer.encoder.get_reference_points(
            ...
            self.occ_ydim,
            self.occ_zdim,
            num_points_in_pillar=self.occ_zdim,
            bs=bs,
        )  # (bs, num_points_in_pillar, H*W, 3)

        ref_3d = ref_3d.view(
            bs, self.occ_zdim, self.occ_xdim, self.occ_ydim, 3
        )  # TODO: order of xy
        ...
        for i in range(1, seq_len):
            ...
            backward_occupancy = F.grid_sample(
                occ_pred_prev, backward_flow, padding_mode="zeros"
            )
            backward_occupancy = backward_occupancy.permute(0, 2, 3, 4, 1)
            w = torch.rand(
                (1, 1, 1, 1, 1),
                dtype=backward_occupancy.dtype,
                device=backward_occupancy.device,
            )
            occ_pred_backward[:, i] = self.flow_fc(
                occ_pred[:, i] * (1 - w) + backward_occupancy * w
            )
        ...
        for i in range(seq_len - 1)[::-1]:
            ...
            forward_occupancy = F.grid_sample(
                occ_pred_next, forward_flow, padding_mode="zeros"
            )
            forward_occupancy = forward_occupancy.permute(0, 2, 3, 4, 1)
            w = torch.rand(
                (1, 1, 1, 1, 1),
                dtype=forward_occupancy.dtype,
                device=forward_occupancy.device,
            )
            occ_pred_forward[:, i] = self.flow_fc(
                occ_pred[:, i] * (1 - w) + forward_occupancy * w
            )
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None, only_bev=False):
        ...
        Args:
            ...
            only_bev: only compute BEV features with encoder. # noqa:E501
        Returns:
        ...
        if only_bev:  # only use encoder to obtain BEV features
            ...
            if self.occ_head_type == "mlp":
                if self.use_fine_occ:
                    ...
                    occ_pred = self.up_sample(
                        occ_pred
                    )  # (bs*seq_len, C, occ_z, occ_y, occ_x)
                    occ_pred = occ_pred.reshape(
                        bs * seq_len, -1, self.occ_dims
                    )  # noqa:E501
                    ...
                    occ_pred = occ_pred.permute(
                        1, 2, 0, 3
                    )  # bs*seq_len, z, x*y, dim # noqa:E501
                    if self.with_occupancy_flow:
                        occ_pred = self.occupancy_aggregation(
                            occ_pred.view(
                                bs,
                                seq_len,
                                self.occ_zdim,
                ...
                bev_for_occ = bev_for_occ.view(
                    bs, 1, self.bev_h, self.bev_w, self.embed_dims
                )
                bev_for_occ = bev_for_occ.permute(0, 2, 3, 1, 4).flatten(
                    3
                )  # (bs, bev_h, bev_w, -1)
                occ_pred = self.occ_proj(bev_for_occ)
                if self.use_fine_occ:
                    ...
                    occ_pred = occ_pred.permute(
                        0, 4, 3, 1, 2
                    )  # (bs, occ_dims, z_dim, bev_h, bev_w)
                outputs_occupancy, outputs_flow = self.occ_seg_head(occ_pred)
                outputs_occupancy = outputs_occupancy.reshape(
                    bs, self.occupancy_classes, -1
                )
            ...
            # bev_embed = bev_embed.permute(1, 0, 2)  # (num_query, bs, embed_dims) # noqa:E501
            outs = {
                "bev_embed": bev_embed,
                "all_cls_scores": None,
                "all_bbox_preds": None,
        ...
        occ_pred = self.occ_proj(bev_for_occ)
        occ_pred = occ_pred.view(
            self.bev_h * self.bev_w, bs * seq_len, self.occ_zdim, self.occ_dims
        )
        ...
        hs = hs.permute(
            0, 2, 1, 3
        )  # (num_dec_layers, bs, num_query, embed_dims) # noqa:E501
    ...
    def loss_single(
        ...
        Args:
            ...
            bbox_preds (Tensor): Sigmoid outputs from a single decoder layer
                ...
                for 3d: shape is [bs, num_query, 10]   (cx, cy, w, l, cz, h, sin(theta), cos(theta), vx, vy) # noqa:E501
            gt_bboxes_list (list[Tensor]): Ground truth bboxes for each image
                with shape (num_gts, 4) in [tl_x, tl_y, br_x, br_y] format.
        ...
        if occupancy_preds is not None:
            ...
            loss_occupancy = self.loss_occupancy(
                occupancy_preds, gt_occupancy, avg_factor=occ_avg_factor
            )
        else:
        ...
        if flow_preds is not None:
            ...
            loss_flow = self.loss_flow(
                flow_preds[object_mask],
                gt_flow[object_mask],
                avg_factor=flow_avg_factor,
            )
        else:
    ...
    def loss(
        ...
        Args:
            ...
            gt_labels_list (list[Tensor]): Ground truth class indices for each
                image with shape (num_gts, ).
            point_coords: list(list(Tensor)): index of occupied voxel  Tensor.shape: (num_points, 4) # noqa:E501
            preds_dicts:
        ...
        all_cls_scores = preds_dicts[
            "all_cls_scores"
        ]  # (num_dec_layers, bs, num_query, 10)
        all_bbox_preds = preds_dicts["all_bbox_preds"]
        occupancy_preds = preds_dicts[
            "occupancy_preds"
        ]  # (bs, occ_zdim*occ_ydim*occ_xdim, 11)
        flow_preds = preds_dicts[
            "flow_preds"
        ]  # (bs, occ_zdim*occ_ydim*occ_xdim, 2) # noqa:E501
        ...
        if isinstance(gt_bboxes_list[0], list):
            ...
            seq_len = 1
            temporal_gt_bboxes_list = [
                [gt_bboxes] for gt_bboxes in gt_bboxes_list
            ]  # noqa:E501
            # temporal_gt_labels_list = [[gt_labels] for gt_labels in gt_labels_list] # noqa:E501
        ...
        if occ_gts:
            gt_occupancy = (
                torch.ones((bs * seq_len, self.voxel_num), dtype=torch.long)
            ...
            for sample_id in range(len(temporal_gt_bboxes_list)):
                for frame_id in range(seq_len):
                    ...
                    gt_occupancy[sample_id * seq_len + frame_id][
                        [occ_gt[:, 0]]
                    ] = occ_gt[:, 1]

            if flow_preds is not None:
                gt_flow = torch.zeros((bs * seq_len, self.voxel_num, 2)).to(
                    device
                )  # noqa:E501
                for sample_id in range(len(temporal_gt_bboxes_list)):
                    for frame_id in range(seq_len):
                        occ_gt = occ_gts[sample_id][frame_id].long()
                        flow_gt_sparse = flow_gts[sample_id][
                            frame_id
                        ]  # only store the flow of occupied grid
                        gt_flow[sample_id * seq_len + frame_id][
                            [occ_gt[:, 0]]
                        ] = flow_gt_sparse
        ...
        all_gt_occupancy_list = [None for _ in range(num_dec_layers - 1)] + [
            gt_occupancy
        ]
        all_occupancy_preds = [None for _ in range(num_dec_layers - 1)] + [
            occupancy_preds
        ]
        all_gt_flow_list = [None for _ in range(num_dec_layers - 1)] + [
            gt_flow
        ]  # noqa:E501
        all_flow_preds = [None for _ in range(num_dec_layers - 1)] + [
            flow_preds
        ]  # noqa:E501
        ...
        if enc_cls_scores is not None:
            ...
            (
                enc_loss_cls,
                enc_losses_bbox,
                enc_losses_occupancy,
            ) = self.loss_single(  # noqa:E501
                enc_cls_scores,
                enc_bbox_preds,
                enc_occupancy_preds,
                gt_bboxes_list,
    ...
    def loss_only_occupancy(
        ...
        occupancy_preds = preds_dicts[
            "occupancy_preds"
        ]  # (bs, occ_zdim*occ_ydim*occ_xdim, 16)
        flow_preds = preds_dicts[
            "flow_preds"
        ]  # (bs, occ_zdim*occ_ydim*occ_xdim, 2) # noqa:E501
        ...
        gt_occupancy = gt_occupancy.view(
            -1
        )  # (bs*seq_len*occ_zdim*occ_ydim*occ_xdim) # noqa:E501
        occupancy_preds = occupancy_preds.view(
            -1, self.occupancy_classes
        )  # (bs*seq_len*occ_zdim*occ_ydim*occ_xdim, 16)
        ...
        loss_occupancy = self.loss_occupancy(
            occupancy_preds,
            gt_occupancy,
            avg_factor=occ_avg_factor,
            weight=occ_pred_weights,
        )
    ...
    @force_fp32(apply_to=("preds_dicts"))
    def loss_semantic_kitti(self, occ_gts, preds_dicts, img_metas=None):
        """"Loss function.
        """
        occupancy_preds = preds_dicts[
            "occupancy_preds"
        ]  # (bs, occ_zdim*occ_ydim*occ_xdim, occ_num)
        ...
        if self.occ_loss_type == "focal_loss":
            ...
            # add occ weights
            if self.occ_weights is not None:
                weights = torch.Tensor(self.occ_weights).to(
                    gt_occupancy.device
                )  # noqa:E501
                occ_pred_weights = weights[gt_occupancy]
            else:
            ...
            class_weights = class_weights.to(occupancy_preds.dtype).to(
                occupancy_preds.device
            )
    ...
    def get_occupancy_prediction(self, occ_results, occ_threshold=0.25):
        """
        occupancy_preds: (Tensor): (bs, occ_zdim*occ_ydim*occ_xdim, occupancy_classes) # noqa:E501
        ...
        if self.occ_loss_type == "focal_loss":
            occupancy_preds = occupancy_preds.reshape(
                -1, self.occupancy_classes
            )  # noqa:E501
            ...
            occupancy_preds = torch.cat(
                (
                    occupancy_preds,
                    torch.ones_like(occupancy_preds)[:, :1] * occ_threshold,
                ),
                dim=-1,
            )
