...
# GUI adapted from
# https://memotut.com/create-a-3d-model-viewer-with-pyqt5-and-pyqtgraph-b3916/ and # noqa:E501
...
from fog_simulation import RNG, ParameterSet, simulate_fog
from plyfile import PlyData
from PyQt5.QtCore import Qt
from PyQt5.QtGui import QComboBox, QDragEnterEvent, QDropEvent, QLineEdit
...
from SeeingThroughFog.tools.DatasetFoggification.beta_modification import (
    BetaRadomization,
)
from SeeingThroughFog.tools.DatasetFoggification.lidar_foggification import (
    haze_point_cloud,
)
from SeeingThroughFog.tools.DatasetViewer.dataset_viewer import (
    load_calib_data,
    read_label,
)
...
parser.add_argument(
    ...
    "--datasets",
    type=str,
    help="path to where you store your datasets",
...
parser.add_argument(
    ...
    "--experiments",
    type=str,
    help="path to where you store your OpenPCDet experiments",
...
FOG = (
    DATASETS_ROOT
    / "DENSE/SeeingThroughFog/lidar_hdl64_strongest_fog_extraction"  # noqa:E501
)
...
if socket.gethostname() == "beast":
    DENSE = (
        Path.home()
        / "datasets_local"
        / "DENSE/SeeingThroughFog/lidar_hdl64_strongest"  # noqa:E501
    )
...
class MyWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.toggle_extracted_fog_btn.clicked.connect(
            self.toggle_extracted_fog
        )  # noqa:E501
        self.layout.addWidget(self.toggle_extracted_fog_btn, 8, 3, 1, 2)

        if self.simulated_fog:
            self.toggle_simulated_fog_btn = QPushButton(
                "remove our fog simulation"
            )  # noqa:E501
        else:
            self.toggle_simulated_fog_btn = QPushButton(
                "add our fog simulation"
            )  # noqa:E501

        self.toggle_simulated_fog_btn.clicked.connect(
            self.toggle_simulated_fog
        )  # noqa:E501
        ...
        self.mor_label = QLabel(
            f"meteorological optical range (MOR) = {round(self.p.mor, 2)}m"
        )
        ...
        self.alpha_slider.setMinimum(
            int(self.p.alpha_min * self.p.alpha_scale)
        )  # noqa:E501
        self.alpha_slider.setMaximum(
            int(self.p.alpha_max * self.p.alpha_scale)
        )  # noqa:E501
        ...
        self.beta_label = QLabel(
            f"\u03B2 = {round(self.p.beta * self.p.mor, 3)} / MOR"
        )  # noqa:E501
        ...
        self.gamma_slider.setMinimum(
            int(self.p.gamma_min * self.p.gamma_scale)
        )  # noqa:E501
        self.gamma_slider.setMaximum(
            int(self.p.gamma_max * self.p.gamma_scale)
        )  # noqa:E501
        ...
        if self.simulated_fog_dense:
            self.toggle_simulated_fog_dense_btn = QPushButton(
                "remove STF fog simulation"
            )
        else:
            self.toggle_simulated_fog_dense_btn = QPushButton(
                "add STF fog simulation"
            )  # noqa:E501

        self.toggle_simulated_fog_dense_btn.clicked.connect(
            self.toggle_simulated_fog_dense
        )
        self.layout.addWidget(
            self.toggle_simulated_fog_dense_btn, self.current_row, 3, 1, 2
        )
        self.toggle_simulated_fog_dense_btn.setEnabled(False)

        # Create textbox
        if self.show_predictions:
            self.visualize_predictions_path_btn = QPushButton(
                "hide predictions", self
            )  # noqa:E501
        else:
            self.visualize_predictions_path_btn = QPushButton(
                "show predictions", self
            )  # noqa:E501
        self.visualize_predictions_path_btn.setEnabled(False)
        self.layout.addWidget(
            self.visualize_predictions_path_btn, self.current_row, 0, 1, 1
        )
        self.visualize_predictions_path_btn.clicked.connect(
            self.toggle_predictions
        )  # noqa:E501
        self.experiment_path_box = QLineEdit(self)
        self.experiment_path_box.setText(
            "dense_models/pv_rcnn/"
            "2021-03-13_01-29-41_98d97b57_dense_models_batch_size_24_pv_rcnn"
        )
        self.layout.addWidget(
            self.experiment_path_box, self.current_row, 1, 1, 1
        )  # noqa:E501
        self.load_experiment_path_btn = QPushButton("load results", self)
        self.layout.addWidget(
            self.load_experiment_path_btn, self.current_row, 2, 1, 1
        )  # noqa:E501
        ...
        self.layout.addWidget(
            self.prediction_threshold_title, self.current_row, 0
        )  # noqa:E501

        self.prediction_threshold_label = QLabel(
            str(self.prediction_threshold)
        )  # noqa:E501
        self.prediction_threshold_label.setAlignment(Qt.AlignCenter)
        self.layout.addWidget(
            self.prediction_threshold_label, self.current_row, 2
        )  # noqa:E501
        ...
        self.prediction_threshold_slider.setValue(
            self.prediction_threshold
        )  # noqa:E501
        self.prediction_threshold_slider.setTickPosition(
            QSlider.TicksBelow
        )  # noqa:E501
        ...
        self.layout.addWidget(
            self.prediction_threshold_slider, self.current_row, 1
        )  # noqa:E501
        self.prediction_threshold_slider.valueChanged.connect(
            self.prediction_threshold_slider_change
        )
    ...
    def load_results(self) -> None:
        ...
        test_folders = [
            x[0]
            for x in os.walk(exp_dir)
            if "epoch" in x[0] and "test" in x[0]  # noqa:E501
        ]
        self.result_dict = {}
    ...
    def visualize_predictions(self) -> None:
        ...
        if "test" in split:
            ...
            assert (
                self.file_name.split("/")[-1].split(".")[0]
                == pred_dict["frame_id"]  # noqa:E501
            ), (
                f"frame missmatch "
                f"{self.file_name.split('/')[-1].split('.')[0]} != {pred_dict['frame_id']}"  # noqa:E501
            )
            lookup = {"Car": 0, "Pedestrian": 1, "Cyclist": 2}
            predictions = np.zeros((pred_dict["boxes_lidar"].shape[0], 9))
            predictions[:, 0:-2] = pred_dict["boxes_lidar"]
            predictions[:, 7] = np.array(
                [lookup[name] for name in pred_dict["name"]]
            )  # noqa:E501
            predictions[:, 8] = pred_dict["score"]

            for prediction in predictions:
                x, y, z, w, l, h, rotation, category, score = prediction  # noqa:E501
                if score * 100 > self.prediction_threshold:
                    dist = np.sqrt(x ** 2 + y ** 2 + z ** 2)
                    rotation = np.rad2deg(rotation) + 90
                    color = (255, 255, 255, 255)  # white
                    box = gl.GLBoxItem(
                        QtGui.QVector3D(1, 1, 1), color=color
                    )  # noqa:E501
                    box.setSize(l, w, h)
    ...
    def populate_dense_splits(self) -> List[str]:
        split_folder = (
            Path(__file__).parent.absolute() / "SeeingThroughFog" / "splits"
        )  # noqa:E501
        splits = []
    ...
    def toggle_simulated_fog(self) -> None:
        if self.file_list is not None and "extraction" not in self.file_name:
            ...
            if self.simulated_fog:
                self.toggle_simulated_fog_btn.setText(
                    "remove our fog simulation"
                )  # noqa:E501
                self.alpha_slider.setEnabled(True)
                self.beta_slider.setEnabled(True)
    ...
    def toggle_simulated_fog_dense(self) -> None:
        if self.file_list is not None and "extraction" not in self.file_name:
            ...
            if self.simulated_fog_dense:
                self.toggle_simulated_fog_dense_btn.setText(
                    "remove STF fog simulation"
                )  # noqa:E501
                self.alpha_slider.setEnabled(True)
                self.beta_slider.setEnabled(True)
    ...
    def create_file_list(
        self, dirname: str, filename: str = None, extension: str = None
    ) -> None:
        if extension:
            file_list = [
                y
            ...
            file_list = [
                y
                for x in os.walk(dirname)
                for y in glob(os.path.join(x[0], f"*.{self.extension}"))
            ]
        ...
        #                           f'{Path(file).name}\n' for file in self.file_list) # noqa:E501
    ...
    def show_pcdet_dict(self, filename: str) -> None:
        ...
        mesh = gl.GLScatterPlotItem(
            pos=np.asarray(pc[:, 0:3]), size=self.point_size, color=colors
        )
    ...
    def create_boxes(self, annotations):
        ...
        for annotation in annotations:
            ...
            l1 = gl.GLLinePlotItem(
                pos=pts, width=2 / 3, color=color, antialias=True, mode="lines"
            )
    ...
    def show_pointcloud(self, filename: str) -> None:
        ...
        if (
            self.extracted_fog
            and self.success
            and "extraction" not in filename  # noqa:E501
        ):
            self.toggle_simulated_fog_btn.setEnabled(False)
            self.toggle_simulated_fog_dense_btn.setEnabled(False)
            ...
            fog = gl.GLScatterPlotItem(
                pos=np.asarray(fog_points[:, 0:3]),
        ...
        if (
            ...
            self.toggle_simulated_fog_dense_btn.setEnabled(False)
            self.reset_viewer()
            pc, simulated_fog_pc, info_dict = simulate_fog(
                self.p,
                self.current_pc,
                self.noise,
                self.gain,
                self.noise_variant,  # noqa:E501
            )
        ...
        if (
            ...
            arguments = Namespace(
                sensor_type="Velodyne HDL-64E S3D", fraction_random=0.05
            )
    ...
    def populate_dense_boxes(self, filename):
        ...
        root = (
            str(Path.home())
            + "/repositories/PCDet/lib/SeeingThroughFog/tools/DatasetViewer/calibs"  # noqa:E501
        )
        ...
        label_path = (
            Path(filename).parent.parent / "gt_labels" / "cam_left_labels_TMP"
        )  # noqa:E501

        # here without '.txt' as it will be added in read_label function
        recording = Path(filename).stem
        label_file = os.path.join(label_path, recording)
        label = read_label(
            label_file, label_path, camera_to_velodyne=camera_to_velodyne_rgb
        )
        ...
        for annotation in label:
            ...
            if annotation["identity"] in [
                ...
                box.rotate(
                    angle=-annotation["rotz"] * 180 / 3.14159265359,
                    x=0,
                    y=0,
                    z=1,  # noqa:E501
                )
                box.rotate(
                    angle=-annotation["roty"] * 180 / 3.14159265359,
                    x=0,
                    y=1,
                    z=0,  # noqa:E501
                )
                box.rotate(
                    angle=-annotation["rotx"] * 180 / 3.14159265359,
                    x=1,
                    y=0,
                    z=0,  # noqa:E501
                )
    ...
    def log_string(self, pc: np.ndarray) -> None:
        ...
        log_string = (
            "max_dist "
            + f"{int(self.p.r_range)}".rjust(3, " ")
            + " m | "
            + "intensity [ "
            + f"{int(min(pc[:, 3]))}".rjust(3, " ")
            + ", "
            + f"{int(max(pc[:, 3]))}".rjust(3, " ")
            + "]"
            + " "
            + "median "
            + f"{int(np.round(np.median(pc[:, 3])))}".rjust(3, " ")
            + " "
            + "mean "
            + f"{int(np.round(np.mean(pc[:, 3])))}".rjust(3, " ")
            + " "
            + "std "
            + f"{int(np.round(np.std(pc[:, 3])))}".rjust(3, " ")
        )

        if self.num_fog_responses > 0:
            range_fog_response_string = (
                "fog [ "
                + f"{int(self.min_fog_response)}".rjust(3, " ")
                + ", "
                + f"{int(self.max_fog_response)}".rjust(3, " ")
                + "]"
            )
            num_fog_responses_string = (
                "num_soft " + f"{int(self.num_fog_responses)}".rjust(6, " ")
            )
            num_remaining_string = (
                "num_hard "
                + f"{int(len(self.current_pc) - self.num_fog_responses)}".rjust(  # noqa:E501
                    6, " "
                )
            )
    ...
    def load_fog_points(self, index: int = None) -> np.ndarray:
        ...
        if index is None and self.extracted_fog_index == -1:
            index = RNG.integers(
                low=0, high=len(self.extracted_fog_file_list), size=1
            )[  # noqa:E501
                0
            ]
    ...
    def load_pointcloud(self, filename: str) -> np.ndarray:
        ...
        if self.dataset == "Honda3D":
            ...
            self.file_name_label.setText(
                f"{Path(filename).parent.name}/" f"{Path(filename).name}"
            )

        elif self.dataset == "PandaSet" or self.dataset == "Apollo":

            self.file_name_label.setText(
                f"{Path(filename).parent.parent.name}/"
    ...
    def dragEnterEvent(self, e: QDragEnterEvent) -> None:
        ...
        if "text/uri-list" in mimeList:
            ...
            filename = (
                ...
                .replace("\r\n", "")
                .replace("%20", " ")  # noqa:E501
            )
            filename = Path(filename)

        if filename.exists() and (
            filename.suffix == ".bin"
            or filename.suffix == ".ply"
            or filename.suffix == ".pickle"
        ):
            e.accept()
    ...
    def dropEvent(self, e: QDropEvent) -> None:
        if self.droppedFilename:
            self.create_file_list(
                Path(self.droppedFilename).parent, self.droppedFilename
            )
